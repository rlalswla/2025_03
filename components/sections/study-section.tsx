  ...
  return (
    ...
    >
      ...
      <div className="container relative z-10 max-w-5xl px-4 mx-auto">
        <h2 className="mb-12 text-3xl font-bold text-center sm:text-4xl">Study Storage</h2>
        <div className="grid gap-8 md:grid-cols-2">
          {studyItems.map((item, index) => (
            <div
              key={item.id}
              onClick={() => handleCardClick(item.url)}
              className="p-6 transition-all duration-500 rounded-lg hover:shadow-lg hover:shadow-primary/10 hover:-translate-y-2 bg-background/70 backdrop-blur-md border border-border cursor-pointer h-[280px] flex flex-col"
              style={{ transitionDelay: `${index * 100}ms` }}
            >
              <h3 className="mb-2 text-xl font-semibold hover:text-primary transition-colors duration-300">
                {item.title}
              </h3>
              <p className="mb-4 text-muted-foreground overflow-hidden line-clamp-4">{item.description}</p>
